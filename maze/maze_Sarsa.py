...
# TD error
def Sarsa(state, action, reward, next_state, next_action, Q, eta, gamma):
    if next_state==8:
...
def goal_maze(Q, epsilon, eta, gamma, pi):
    ...
    while 1:
        ...
        Q = Sarsa(state, action, reward, next_state, next_action, Q, eta, gamma)
        if next_state==8:
            break
        else:
            state = next_state
...
def plot_history(fig, plt, line, step_text, state_action_history):
    ...
    anim.save('maze_Sarsa.gif', writer='pillow') # imagemagick
    # HTML(anim.to_jshtml())
    plt.show()
...
if __name__ == '__main__':
    ...
    Q, epsilon, eta, gamma = value_strategy(pi_0)
